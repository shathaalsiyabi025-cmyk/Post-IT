...
const Location = () =>{
  ...
  const getGeoLocationData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://geo.ipify.org/api/v2/country?apiKey=at_hLX3rngVbIZ1IHWiSkeVLDqgafmiU=${ip}`
      );
  ...
  const getGeoLocationData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://geo.ipify.org/api/v2/country?apiKey=PLACE YOUR API KEY HERE&ipAddress=${ip}`
      );
      setGeoData(response.data); // Set geolocation data in state
      console.log("GeoLocation Data:", response.data);
    } catch (error) {
